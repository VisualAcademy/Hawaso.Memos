...
namespace Hawaso.Models
{
    ...
    /// </summary>
    public class InquiryRepository : IInquiryRepository, IDisposable
    {
        private readonly InquiryAppDbContext _context;
        private readonly ILogger _logger;

        public InquiryRepository(InquiryAppDbContext context, ILoggerFactory loggerFactory)
        {
            this._context = context;
            this._logger = loggerFactory.CreateLogger(nameof(InquiryRepository));
        }

        #region [4][1] 입력: AddAsync
        //[4][1] 입력: AddAsync
        public async Task<Inquiry> AddAsync(Inquiry model)
        {
            #region 답변 기능 추가
            // 현재테이블의 Ref의 Max값 가져오기
            int maxRef = 1;
            int? max = await _context.Inquiries.DefaultIfEmpty().MaxAsync(m => m == null ? 0 : m.Ref);
            if (max.HasValue)
            {
                maxRef = (int)max + 1;
            }
        ...
        {
            // 학습 목적으로... InInquiryry 데이터베이스에선 사용 금지 
            //return await _context.Inquiries.FromSqlRaw<Inquiry>("Select * From dbo.Inquiries Order By Id Desc") 
            return await _context.Inquiries.OrderByDescending(m => m.Id)
                //.Include(m => m.InquiriesComments)
                .ToListAsync();
        }
        #endregion
        ...
        //[4][3] 상세: GetByIdAsync
        public async Task<Inquiry> GetByIdAsync(long id)
        {
            var model = await _context.Inquiries
                //.Include(m => m.InquiriesComments)
            ...
            if (model != null)
            {
                ...
                _context.Inquiries.Attach(model);
        ...
        //[4][4] 수정: UpdateAsync
        public async Task<bool> EditAsync(Inquiry model)
        {
            try
            {
        ...
        public async Task<bool> UpdateAsync(Inquiry model)
        {
            try
            {
                var old = _context.Inquiries.Find(model.Id);
        ...
        {
            //var model = await _context.Inquiries.SingleOrDefaultAsync(m => m.Id == id);
            try
            {
                var model = await _context.Inquiries.FindAsync(id);
        ...
        //[4][6] 페이징: GetAllAsync()
        public async Task<PagingResult<Inquiry>> GetAllAsync(int pageIndex, int pageSize)
        {
            var totalRecords = await _context.Inquiries.CountAsync();
            var models = await _context.Inquiries
            ...
            return new PagingResult<Inquiry>(models, totalRecords);
        } 
        #endregion

        //[4][7] 부모
        public async Task<PagingResult<Inquiry>> GetAllByParentIdAsync(
            int pageIndex,
            int pageSize,
            int parentId)
        {
            var totalRecords = await _context.Inquiries
        ...
        public async Task<Tuple<int, int>> GetStatus(int parentId)
        {
            var totalRecords = await _context.Inquiries
                .Where(m => m.ParentId == parentId)
                .CountAsync();
            var pinnedRecords = await _context.Inquiries
        ...
        //[4][10] 검색
        public async Task<PagingResult<Inquiry>> SearchAllAsync(
            int pageIndex,
            int pageSize,
            string searchQuery)
        {
        ...
        //[4][11] 부모 검색
        public async Task<PagingResult<Inquiry>> SearchAllByParentIdAsync(
            int pageIndex,
            int pageSize,
            string searchQuery,
            int parentId)
        ...
        public async Task<SortedList<int, double>> GetMonthlyCreateCountAsync()
        {
            ...
            for (int i = 0; i < 12; i++)
            {
                ...
                var cnt = _context.Inquiries.AsEnumerable()
                    .Where(
                        m => m.Created != null
                        &&
        ...
        //[4][13] 부모 페이징
        public async Task<PagingResult<Inquiry>> GetAllByParentKeyAsync(
            int pageIndex,
            int pageSize,
            string parentKey)
        {
        ...
        //[4][14] 부모 검색
        public async Task<PagingResult<Inquiry>> SearchAllByParentKeyAsync(
            int pageIndex,
            int pageSize,
            string searchQuery,
            string parentKey)
        ...
        //[4][15] 리스트(페이징, 검색, 정렬)
        public async Task<ArticleSet<Inquiry, int>> GetArticlesAsync<TParentIdentifier>(
            int pageIndex,
            int pageSize,
            string searchField,
            string searchQuery,
        ...
        {
            //var items = from m in _context.Inquiries select m; // 쿼리 구문(Query Syntax)
            //var items = _context.Inquiries.Select(m => m); // 메서드 구문(Method Syntax)
            var items = 
                _context.Inquiries
            ...
            return new ArticleSet<Inquiry, int>(await items.AsNoTracking().ToListAsync(), totalCount);
        }

        //[4][16] 답변: ReplyApp
        public async Task<Inquiry> AddAsync(Inquiry model, int parentRef, int parentStep, int parentRefOrder)
        {
            #region 답변 관련 기능 추가된 영역
            // 비집고 들어갈 자리: 부모글 순서보다 큰 글이 있다면(기존 답변 글이 있다면) 해당 글의 순서를 모두 1씩 증가 
            var replys = await _context.Inquiries
            ...
            foreach (var item in replys)
            {
                ...
                try
                {
                    _context.Inquiries.Attach(item);
        ...
        //[4][17] 답변: InquiryApp
        public async Task<Inquiry> AddAsync(Inquiry model, int parentId)
        {
            #region 답변 관련 기능 추가된 영역
            //[0] 변수 선언
            var maxRefOrder = 0;
            ...
            var tempMaxRefOrder = await _context.Inquiries.Where(m => m.ParentNum == parentId).DefaultIfEmpty().MaxAsync(m => m == null ? 0 : m.RefOrder);
            var sameGroup = await _context.Inquiries.Where(m => m.ParentNum == parentId && m.RefOrder == tempMaxRefOrder).FirstOrDefaultAsync();
            if (sameGroup != null)
            {
            ...
            {
                var tmpParent = await _context.Inquiries.Where(m => m.Id == parentId).SingleOrDefaultAsync();
                if (tmpParent != null)
                {
            ...
            var replys = await _context.Inquiries.Where(m => m.Ref == parentRef && m.RefOrder > (maxRefOrder + maxAnswerNum)).ToListAsync();
            foreach (var item in replys)
            {
                item.RefOrder = item.RefOrder + 1;
                try
        ...
        //[4][6] 검색: GetByAsync()
        public async Task<ArticleSet<Inquiry, long>> GetByAsync<TParentIdentifier>(
            FilterOptions<TParentIdentifier> options)
        {
            //var items = from m in _context.Inquiries select m; // 쿼리 구문(Query Syntax)
            //var items = _context.Inquiries.Select(m => m); // 메서드 구문(Method Syntax)
            var items = _context.Inquiries.AsQueryable();
            ...
            return new ArticleSet<Inquiry, long>(await items.AsNoTracking().ToListAsync(), totalCount);
        }
        #endregion
