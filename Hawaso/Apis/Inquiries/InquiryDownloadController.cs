...
{
    [Authorize]
    public class InquiryDownloadController : Controller
    {
        private readonly IInquiryRepository _repository;
        private readonly IInquiryFileStorageManager _fileStorageManager;

        public InquiryDownloadController(IInquiryRepository repository, IInquiryFileStorageManager fileStorageManager)
        {
        ...
        public async Task<IActionResult> FileDown(int id)
        {
            ...
            else
            {
                ...
                {
                    byte[] fileBytes = await _fileStorageManager.DownloadAsync(model.FileName, "Inquiries");
                    if (fileBytes != null)
                    {
        ...
        public async Task<IActionResult> ExcelDown()
        {
            ...
            if (models != null)
            {
                using (var package = new ExcelPackage())
                {
                    ...
                    return File(package.GetAsByteArray(), "application/octet-stream", $"{DateTime.Now.ToString("yyyyMMddhhmmss")}_Inquiries.xlsx");
                }
            }
            return Redirect("/");
        }
