...
namespace Hawaso.Pages.Inquiries
{
    public partial class Manage
    {
        ...
        [Inject]
        public IInquiryFileStorageManager FileStorageManagerReference { get; set; }
        #endregion
        ...
        /// </summary>
        protected List<Inquiry> models = new List<Inquiry>();

        /// <summary>
        /// 현재 페이지에서 선택된 단일 데이터를 나타내는 모델 클래스 
        /// </summary>
        protected Inquiry model = new Inquiry();
        ...
        {
            NavigationManagerInjector.NavigateTo($"/Inquiries/Details/{id}");
        }
        ...
        /// </summary>
        protected void EditBy(Inquiry model)
        {
        ...
        /// </summary>
        protected void DeleteBy(Inquiry model)
        {
            this.model = model;
        ...
        protected async void DownloadBy(Inquiry model)
        {
            if (!string.IsNullOrEmpty(model.FileName))
            {
                byte[] fileBytes = await FileStorageManagerReference.DownloadAsync(model.FileName, "Inquiries");
                if (fileBytes != null)
                {
        ...
        protected async void DeleteClick()
        {
            ...
            {
                // 첨부 파일 삭제 
                await FileStorageManagerReference.DeleteAsync(model.FileName, "Inquiries");
            }

            await RepositoryReference.DeleteAsync(this.model.Id);
            DeleteDialogReference.Hide();
            this.model = new Inquiry(); // 선택했던 모델 초기화
        ...
        protected async void ToggleClick()
        {
            ...
            this.model = new Inquiry(); // 선택한 모델 초기화 
        ...
        /// ToggleBy(PinnedBy)
        /// </summary>
        protected void ToggleBy(Inquiry model)
        {
            this.model = model;
        ...
        {
            FileUtil.SaveAsExcel(JSRuntimeInjector, "/InquiryDownload/ExcelDown");

            NavigationManagerInjector.NavigateTo($"/Inquiries"); // 다운로드 후 현재 페이지 다시 로드
        }
        ...
        protected void DownloadExcel()
        {
            ...
            {
                var worksheet = package.Workbook.Worksheets.Add("Inquiries");
                ...
                FileUtil.SaveAs(JSRuntimeInjector, $"{DateTime.Now.ToString("yyyyMMddhhmmss")}_Inquiries.xlsx", package.GetAsByteArray());
            }
        }
        #endregion
